...
1. 外接硬盘的分区格式，之前在 Windows 笔记本上常用的一个移动硬盘只有 USB 的 A 口，所以只能通过扩展坞转成 C 口再接到 Mac Mini 上，然后有个问题是会发现对这个移动硬盘只能读不能写，一直也没太深究，还以为扩展坞的问题，最近才发现是 Mac OS 对 NTFS 的支持问题。最后是装了一个叫 Pro NTFS 磁盘读写大师的软件来搞定的，过程还算友好，但也不简单，涉及到安装驱动、进维护模式、改系统设置等，以及最终的效果是硬盘接入后还是默认只读，需要在这个工具里重新加载才能变成可写。

    > 网上搜索到的帖子比较多是推荐一个叫赤友的软件，但看上去是有应用内购的，而上面说的这个完全免费，推荐！以及，动手能力强的应该可以不用这些软件，直接自己手搓，毕竟原理都是公开的。
