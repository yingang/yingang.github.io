...
但如果直接在 Terminal 中输入 cargo build 又是可以正常运行的，搞不懂。尝试过回退 VScode 的版本（日常会自动更新），似乎没什么影响，有点怀疑是机器上的信息安全软件又在作祟了（之前已经有一个导致无法在 VSCode 里正常调试 Python 脚本的问题）。
...
* 为了能够调试进 Rust 自带的类库，还是需要用类似于[之前](https://yingang.github.io/rust/vscode/2019/05/02/hello-rust.html)的方法创建一个目录和相应的链接，但对应 Rust 1.41.0 的目录变成了 c:/rustc/5e1a799842ba6ed4a57e91f7ab9435947482f7d8/，然后目标路径也变成 stable-i686-pc-windows-msvc 了：

    mklink /J src C:\Users\gang.yin\.rustup\toolchains\stable-i686-pc-windows-msvc\lib\rustlib\src\rust\src

* F12 可以跳转到类似于 println! 的宏里面，这点比之前有改进，不知道是因为什么导致的。
